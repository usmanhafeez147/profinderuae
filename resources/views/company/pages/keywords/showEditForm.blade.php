			  ...
			  <div class="col-md-12 col-sm-12 col-xs-12">
				<div class="x_panel">
				  ...
				  <div class="x_content">
					...
					<form id="demo-form2" method="post" enctype="multipart/form-data" class="form-horizontal form-label-left" action="{{ route('updateKeywords',$keyword->id)}}">
					{{ csrf_field() }}
					...
					@if(!empty($keyword->name))
					  <div class="form-group {{ $errors->has('name')? 'item bad' : ''}}">
						...
						<div class="col-md-6 col-sm-6 col-xs-12">
						  <input type="text" id="name" value="{{ $keyword->name }}" required="required" class="form-control col-md-7 col-xs-12" name="name">
						</div>
						@if($errors->has('name'))
						...
						<div class="col-md-6 col-sm-6 col-xs-12">
						  <select class="select2_single form-control" name="category" id="category" required="required" >
                            ...
                            @foreach($categories as $category)
                            	@if($category->id==$keyword->category_id)
